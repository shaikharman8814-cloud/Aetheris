...
const documentMemory = new Map(); // memory cache for documents
...
async function startServer() {
    ...
    app.post("/api/chat", dailyLimitMiddleware, async (req, res) => {
        ...
        try {
            ...
            if (attachments && Array.isArray(attachments) && attachments.length > 0) {
                const hasBlockedFile = attachments.some(att => att.type?.startsWith('image/') ||
            ...
            if (rawBase64Images.length > 0) {
                const visionContent = [{ type: "text", text: query }];
            ...
            const getAIResponse = async (msgs, retryOnTokenLimit = true) => {
                let currentResponse;
                // Primary: Try Groq
                ...
                if (usedProvider === "none" && openRouterKey) {
                    ...
                    try {
                        ...
                        if (retryOnTokenLimit && (e.status === 402 || e.message?.toLowerCase().includes("token") || e.code === "context_length_exceeded")) {
                            ...
                            const newMsgs = [{ role: "system", content: getSystemInstruction(mode) }];
                            if (searchContext)
                                newMsgs.push({ role: "system", content: "Use the following web sources to answer the question:\n\n" + searchContext });
                            if (documentContext)
                                newMsgs.push({ role: "system", content: "Relevant document context:\n" + documentContext });
                            newMsgs.push(...reducedHistory.map((msg) => ({
                                role: msg.role === 'assistant' ? 'assistant' : 'user',
                                content: msg.content
                            })));
                            if (rawBase64Images.length > 0) {
    ...
    app.post("/api/title", async (req, res) => {
        ...
        if (!message)
            return res.status(400).json({ title: "" });
        const apiKey = (process.env.GROQ_API_KEY || process.env.OPENROUTER_API_KEY)?.trim();
        if (!apiKey)
            return res.json({ title: message.substring(0, 40) });
    ...
    app.post("/api/share", (req, res) => {
        ...
        if (sharedConversations.size >= 100) {
            ...
            if (oldest)
                sharedConversations.delete(oldest[0]);
        }
