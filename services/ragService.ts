...
class RAGService {
    private store: Chunk[] = [];
    private MAX_CHUNKS_PER_DOC = 300;
    private CHUNK_SIZE = 800;
    private CHUNK_OVERLAP = 150;

    async indexDocument(docId: string, text: string, openai: OpenAI) {
        // Check if document is already indexed to avoid redundant work
        if (this.store.some(c => c.docId === docId)) return;
    ...
    private splitText(text: string): string[] {
        ...
        for (let i = 0; i < text.length; i += (this.CHUNK_SIZE - this.CHUNK_OVERLAP)) {
            ...
            if (chunks.length >= this.MAX_CHUNKS_PER_DOC) break;
        }
        return chunks;
    }
    ...
    async retrieve(query: string, openai: OpenAI, docIds: string[]): Promise<string[]> {
